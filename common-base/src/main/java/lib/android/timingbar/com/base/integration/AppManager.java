...
public class AppManager {
    ...
    public void addActivity(Activity activity) {
        ...
        synchronized (AppManager.class) {
            if (!mActivityList.contains(activity)) {
                ...
                Collections.swap(mActivityList, mActivityList.indexOf(activity), mActivityList.size() - 1);
            }
        }
    }
    ...
    public Activity removeActivity(int location) {
        ...
        synchronized (AppManager.class) {
            if (location > 0 && location < mActivityList.size()) {
                BaseLog.w(TAG, "mActivityList == " + mActivityList.get(location).getClass().getName());
     ...
     * @param location
     */
    public void killActivity(int location) {
        if (mActivityList == null) {
            BaseLog.w(TAG, "mActivityList == null when removeActivity(int)");
            return;
        }
        synchronized (AppManager.class) {
            if (location > 0 && location < mActivityList.size()) {
                BaseLog.w(TAG, "mActivityList == " + mActivityList.get(location).getClass().getName());
                mActivityList.get(location).finish();
            }
        }
    }
